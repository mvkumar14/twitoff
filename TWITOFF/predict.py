...
def predict_user(user1_name,user2_name,tweet_text,cache=None):
    ...
    if cache and cache.exists(user_set):
        ...
        # Get the users from the database (previously loaded in users)
        user1 = User.query.filter(User.name==user1_name).one()
        user2 = User.query.filter(User.name==user2_name).one()

        # Get the embeddings by accessing the tweets attribute of the users
        user1_embeddings = np.array([tweet.embedding for tweet in user1.tweets])
        user2_embeddings = np.array([tweet.embedding for tweet in user2.tweets])

        # Split embeddings into array (to put in model)
